...
// 单次 CPU 使用率采样结果。
// valid=false 表示 warming-up（首次采样无速率值，见 yellow 不变量：无效样本
// 不得冒充零值）；valid=true 时 usagePercent 为 0..100 的当前使用率。
struct CpuSample {
    bool valid = false;
    double usagePercent = 0.0;
};

// PDH CPU 使用率查询（RAII 生命周期）。
...
// 采样间隔过短（毫秒级）会得到失真的 0%/100% 随机值。因此本类在
...
public:
    ...
    // 打开查询并添加计数器。失败时对象保持关闭状态（可安全析构）。
    [[nodiscard]] common::Result<void> Initialize() noexcept;

    // 单次采集。首次调用返回 valid=false（warming-up），第二次起返回有效值；
    // 两次有效采样之间保证 >= minInterval（不足则前台等待）。
    // PDH 错误返回对应 Failure（失败不伪装成功）。
