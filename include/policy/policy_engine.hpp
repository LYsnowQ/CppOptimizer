...
// 策略动作。v1 只读咨询切片：所有动作均为"建议"，绝不触发任何系统修改；
// 执行器（PowerLocker / PriorityBooster / MemoryTuner 清理能力）在后续批次实现。
enum class PolicyAction {
    NoOp,                // 无动作
    Notify,              // 提示用户（危急只提示，不执行）
    SuggestMemoryTune,   // 建议 Layer 2 内存维护（咨询）
    SuggestPriorityBoost // 建议 Layer 3 优先级提升（咨询，v1 规则暂不产生）
};
