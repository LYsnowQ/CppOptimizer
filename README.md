...
- **工程基础**：统一错误域模型（`Result<T>` / `Error`）、RAII 资源所有权、C++20、CTest 单元测试、审计记录（`optimizer::audit`：`--policy` 门禁开启执行 R1 动作时按 docs 审计字段子集记录——operation/risk/caller/target/detail/ok/时刻，进程内有界环形保存并输出窗口审计行；为“审计不可用且请求 R2/R3”Safe Mode 源铺路）
