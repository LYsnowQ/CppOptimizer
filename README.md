...
- **用户输入活动观测**（MOD-ACT-001 首切片）：`--activity <s> [idle-secs]` 前台有界只读观测最近键鼠输入（`GetLastInputInfo`，无 Hook、不采集输入内容）——每秒采样分类 Active/Idle/Unknown（空闲阈值秒数可调，默认 15s；非交互会话查询失败按 Unknown 降级不伪装），输出逐样本状态与窗口汇总；GetTickCount 32 位回绕与阈值边界经纯函数单测覆盖
...
CppOptimizer.exe --activity <s> [idle-secs]  用户输入活动观测（1–60 秒，只读 GetLastInputInfo、无 Hook；空闲阈值 1..3600 秒，默认 15）
