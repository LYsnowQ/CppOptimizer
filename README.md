...
CppOptimizer.exe --service console <s> [config.toml] [--ipc-facts] [--confirm-recovery] [--ipc-token <t>]/[--ipc-token-file [<path>]] [--ipc-allow-user <SID>]  服务宿主控制台模式（前台有界 1–60 秒，R0 只读负载；--ipc-facts 时同时作为受保护管道服务端连续受理 Agent 事实；[config.toml] 经 [ipc] 节配置 Safe Mode 门禁参数——缺省 3 次/5s/冷却 2s，越界或为零配置直接拒绝）；--confirm-recovery（IPC-018）确认上次异常退出（清恢复标记）
