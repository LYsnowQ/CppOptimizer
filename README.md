...
- **每用户自启动项**：`--startup status|install|remove` 在 HKCU Run 下维护每用户自启动项（标准用户、可逆、幂等；`install` 写入启动登录后拉起常驻宿主的完整命令行 `"<exe>" --service console run --tray --ipc-facts`——即登录后随用户会话拉起托盘宿主，同时作为受保护管道 Agent 受理宿主（R0 只读受理/记录/回 Ack），并给回退提示；`remove` 不存在也成功，失败如实报错；默认不自动安装）
- **可选计划任务形态**：`--scheduled-task <status|install|remove> [name]` 以每用户**登录触发**计划任务注册同一命令行（Task Scheduler 2.0，额外具备“错过触发后补跑”等调度语义）；**注册需管理员（一次性 UAC），任务本身永不提权**——以当前用户交互令牌运行，不设置最高运行级别；`status` 只读展示任务名/触发方式/命令行；`remove` 幂等；**“开机触发”被显式拒绝**（需以 SYSTEM 运行，与本项目的权限分界冲突）；非提升进程注册会如实报 `拒绝访问` 且不留残留（已实测 `schtasks` 核查无残留）
- **形态入口与状态查询**：`--agent-form <status|install|remove|apply> [startup_tray|task|service] [config.toml] [--replace] [--dry-run]` 统一报告或应用常驻形态（`apply` = **声明式收敛**：读 `[agent].form` 声明的形态，与实态比较后先装声明形态、再卸其它形态；`--dry-run` 只打印计划不执行；每次安装/卸载都写入审计记录）——`status` 只读汇总三形态安装状态（单个形态查询失败只影响它自己的行，如实标注，不冒充未安装）；给出配置路径时另列 `declared`（`[agent].form` 声明的意图）与实态差异，并在声明形态未安装时给出**显式**安装命令；`install`/`remove` 只做**本次动作**、不写配置，各形态保持自身权限语义（启动项标准用户即可、计划任务与服务需管理员注册）；未指定形态一律拒绝，不隐式回退去安装默认形态；`--service status` 额外提供 SCM 服务的只读安装状态（名称/运行状态/启动类型，取不到启动类型时如实显示 `unknown`）；**单一形态生效**——安装时若已有其它形态注册会**拒绝**并提示先卸载，或在显式 `--replace` 下于新形态装好**之后**再移除旧形态（先装后卸，任一时刻至少有一个可用形态）；`--ipc-facts` 的 Agent 受理宿主有**单实例锁**（`Local\CppOptimizerHost_<用户名>`），第二个宿主会如实拒绝启动，避免多宿主争抢同一管道与同一批每用户状态文件
- **形态声明（`[agent].form`）**：配置节受理 `startup_tray`（默认）/`task`/`service` 作为**声明的意图**，未知取值显式拒绝；**配置永不自动注册、永不自动提权**——注册始终是显式命令（一次性 UAC），未配置时恒回退默认形态；需要收敛时运行 `--agent-form apply`（一次显式动作，可用 `--dry-run` 预演）
- **内存清理（计划 / dry-run / R1 真实执行）**：`--memory-clean [config.toml] [--dry-run]` 消费 `[memory].max_clean_level`，
  给出“若门禁开放将要执行什么”的**计划**（light -> 工作集修剪 / medium -> 再加 Standby 清理 / deep -> 再加文件缓存）
  与逐门裁决；dry-run 不调用任何系统调用。`--memory-clean <config.toml> --execute-self-trim
  ...
  **六道门全通才执行**（否则只输出首个阻塞门并拒绝、exit 2）、**审计不可用一律拒绝**、单步单次、不提权、
  ...
  工作集修剪的编译期开关为 `OPTIMIZER_ENABLE_MEMORY_CLEAN`（默认 OFF，与其余三个一样需显式构建开启）；
...
- **电源计划切换（R2，可逆）**：`--power-scheme status|saved` 只读读回当前活动计划 GUID 与保存的恢复依据；
  ...
  `--power-scheme restore` 按保存的 GUID 回滚并读回校验；`--power-scheme recover` 回滚**未确认的切换**
  （恢复依据为每用户 `power-scheme-save.txt`：切换前先写 `state=pending`，读回校验通过才改 `applied`，
  因此崩溃/被杀后 `status` 会显示 `pending`，`recover` 会把它带回保存值）；**回滚路径只要求编译期与审计两道门**
  ——不受配置、确认、环境与冷却门限制（不得把回滚卡在限频上），而正向切换受全部六道门约束
  （每个能力**自己的**编译期开关 `OPTIMIZER_ENABLE_POWER_SCHEME_SWITCH`，默认 OFF）、必须审计可用、
  成功后写冷却台账，三段日记记录前后 GUID。真实“切换→恢复”属隔离环境验证项（宿主不执行）
- **危险能力门禁诊断**：`--gates [config.toml] [--acknowledge-system-wide-side-effects]
  ...
  **命令行门为动作特定确认**：`--acknowledge-system-wide-side-effects` 才打开该门（R3 的两行还需另一条
  `--acknowledge-isolated-environment`，两条缺一不可），通用 `--force` 被明确拒绝（exit 2）——“危险动作必须用可读、动作特定的确认”，不接受一个能匹配所有动作的强制开关；
...
- **动作日记（仅本地存储）**：每个危险/注册类动作按**操作前 → 操作后 → 操作后状态**三段**实时**追加到独立的
  ...
  可压缩，日记回答“改之前是什么、改了什么、改之后是什么”
- **动作审计覆盖面**：`--policy` 的 R1 动作、**直接 CLI 的 R1 动作**（`--power-lock` 的 hold/release、`--priority-boost` 的 boost/unboost）与**注册类动作**（自启动项/计划任务/SCM 服务的安装与卸载，含成功与失败）都会写入每用户审计文件（`operationId` 与策略执行器一致，便于按操作聚合）
- **异常退出恢复门禁**：宿主在 `--ipc-facts` 受理前检查上次会话是否异常退出（恢复标记）；默认暂停 Agent 受理并要求 `--confirm-recovery` 确认；`[application].safe_mode_on_recovery_error = false` 时**不阻断**，但该事实仍会写入日志与横幅（**可见性不可关闭**）
...
Config snapshot (read-only)
  version    : 1
  mode       : observe
  logging    : level info, max 10 MB x 5 backup file(s)
  memory     : query on, clean off, native-write off

> 该输出还会带 `pending` 行（已解析但尚未生效的字段）与 `unknown` 行（被忽略的未知键，通常是拼写错误）；
> 回显字段不代表行为已生效，两者都会如实列出。
