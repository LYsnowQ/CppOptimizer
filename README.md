...
- **Agent 周期上报实体（客户端）**：`--agent run <s> [suffix] [--interval-ms <50..10000>]` 前台有界周期上报——每周期采集真实内存观测并经受保护命名管道上报 FactsSnapshot（可选 agent_token 凭据），请求-应答逐次配对、Error 应答如实计入失败；宿主离线/Safe Mode 暂停等连接失败**有界重试与退避**（单次上报至多 3 次连接尝试），窗口到期必然返回汇总（sent / connect failures），不会无限重连
...
CppOptimizer.exe --agent run <s> [suffix] [--interval-ms <50..10000>]  周期上报客户端（前台有界 1–60 秒；每周期真实内存事实经受保护管道上报，可选 agent_token；连接失败有界重试与退避；窗口结束输出 sent/connect failures 汇总）
