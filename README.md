...
- 规划中：Agent/Service 运行形态（控制台宿主 -> ServiceHost / Per-user Agent）；ServiceHost 首切片已落地（SVC-001），受保护 IPC 传输（IPC-001）、结构化 Facts 载荷（IPC-002）、v1 键语义白名单（IPC-003）、会话级身份裁决（IPC-004）、会话凭据 token（IPC-005，demo 层）与用户 SID 授权白名单（IPC-006）已落地，token 真实供给、真实观测的 Service 端消费与 Agent 实体属后续切片；
