...
- 规划中：Agent/Service 运行形态（控制台宿主 -> ServiceHost / Per-user Agent）；ServiceHost 首切片已落地（SVC-001），受保护 IPC 传输（IPC-001）、结构化 Facts 载荷（IPC-002）、v1 键语义白名单（IPC-003）与会话级身份裁决（IPC-004，默认仅受理交互会话可识别客户端）已落地，token 校验、真实观测的 Service 端消费与 Agent 实体属后续切片；
