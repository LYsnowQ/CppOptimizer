...
- **形态入口与状态查询**：`--agent-form <status|install|remove> [startup_tray|task|service] [config.toml] [--replace]` 统一报告或应用常驻形态——`status` 只读汇总三形态安装状态（单个形态查询失败只影响它自己的行，如实标注，不冒充未安装）；给出配置路径时另列 `declared`（`[agent].form` 声明的意图）与实态差异，并在声明形态未安装时给出**显式**安装命令；`install`/`remove` 只做**本次动作**、不写配置，各形态保持自身权限语义（启动项标准用户即可、计划任务与服务需管理员注册）；未指定形态一律拒绝，不隐式回退去安装默认形态；`--service status` 额外提供 SCM 服务的只读安装状态（名称/运行状态/启动类型，取不到启动类型时如实显示 `unknown`）；**单一形态生效**——安装时若已有其它形态注册会**拒绝**并提示先卸载，或在显式 `--replace` 下于新形态装好**之后**再移除旧形态（先装后卸，任一时刻至少有一个可用形态）；`--ipc-facts` 的 Agent 受理宿主有**单实例锁**（`Local\CppOptimizerHost_<用户名>`），第二个宿主会如实拒绝启动，避免多宿主争抢同一管道与同一批每用户状态文件
- **形态声明（`[agent].form`）**：配置节受理 `startup_tray`（默认）/`task`/`service` 作为**声明的意图**，未知取值显式拒绝；**配置永不自动注册、永不自动提权**——注册始终是显式命令（一次性 UAC），未配置时恒回退默认形态
