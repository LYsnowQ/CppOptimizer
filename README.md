...
- **内存清理（计划与 dry-run 首片）**：`--memory-clean [config.toml] [--dry-run]` 消费 `[memory].max_clean_level`，
  给出“若门禁开放将要执行什么”的**计划**（light -> 工作集修剪）与逐门裁决；**本切片不调用任何系统调用**，
  `--execute` 一律拒绝（R2 真实动作需六道门全通过并在隔离环境验证）；每次计划都写审计与三段动作日记
- **危险能力门禁诊断**：`--gates [config.toml]` 只读列出各危险能力（电源计划切换、计划内存清理、Native 写、
  ...
  并如实打印探测结果（未知即显示 `unknown`，不当作安全）；**审计门为真实可写探测**（只打开不写入，不产生审计记录）；不执行任何动作，也不改变任何门禁状态（R2/R3 真实动作另需隔离环境）
