...
- **形态入口与状态查询**：`--agent-form <status|install|remove> [startup_tray|task|service] [config.toml]` 统一报告或应用常驻形态——`status` 只读汇总三形态安装状态（单个形态查询失败只影响它自己的行，如实标注，不冒充未安装）；给出配置路径时另列 `declared`（`[agent].form` 声明的意图）与实态差异，并在声明形态未安装时给出**显式**安装命令；`install`/`remove` 只做**本次动作**、不写配置，各形态保持自身权限语义（启动项标准用户即可、计划任务与服务需管理员注册）；未指定形态一律拒绝，不隐式回退去安装默认形态；`--service status` 额外提供 SCM 服务的只读安装状态（名称/运行状态/启动类型，取不到启动类型时如实显示 `unknown`）
